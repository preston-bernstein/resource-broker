...
**Embed lane**:
An optional second upstream (ADR-0008): an Infinity SigLIP server fronted on its own port (`:11438`) for the estate-scraper image corpus. Runs on CPU, so it has its own scheduler (no shared GPU slot) but shares the Yield controller — it backs off on Contention like everything else. Presents an OpenAI `/embeddings` face and rewrites to Infinity's `/embeddings_image`. Started only when `INFINITY_URL` is set.
