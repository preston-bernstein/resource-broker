...
go test ./... 2>&1 | tail -8
go vet ./... 2>&1 | tail -4
ssh desktop.example.internal 'systemctl is-active ollama-broker resource-manager; curl -s localhost:11437/status'
```
...
ssh desktop.example.internal 'systemctl is-active ollama-broker resource-manager'
```
