...
6. **Schedule windows are hardcoded in code** (`internal/schedule/schedule.go`: estate-scraper Fri 02:00 + 5h — the only window that pauses Tdarr (`SafeForBackgroundGPU` false) — plus a daily 02:00–09:00 `safe-batch` label marking the PREFERRED slot for background GPU work) — changing the window means editing Go source and redeploying, not config.
