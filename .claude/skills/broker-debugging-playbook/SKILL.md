...
| Thing | Value |
|---|---|
| Desktop LAN IP | `10.0.0.243` (run curl from anywhere on LAN; `journalctl`/`systemctl` only on the desktop) |
...
```bash
curl -s http://10.0.0.243:11437/status | python3 -m json.tool
```
...
```bash
curl -s -X POST http://10.0.0.243:11437/control -d '{"mode":"auto"}'
```
...
```bash
curl -s -X POST http://10.0.0.243:11437/jobs \
...
```bash
curl --raw -is http://10.0.0.243:11435/api/generate \
...
```bash
curl -sN http://10.0.0.243:11435/api/generate \
...
```bash
curl -s http://10.0.0.243:11437/jobs/<id> | python3 -m json.tool   # state, position, attempts
curl -s "http://10.0.0.243:11437/jobs?state=QUEUED"   # states are UPPERCASE; the SQL match is case-sensitive — lowercase silently returns {"jobs":[]}
```
...
So if vectors collapse, the Consumer is almost certainly NOT going through the lane. Correct target: `http://10.0.0.243:11438/embeddings`. Wrong targets that produce this symptom: Infinity `127.0.0.1:7997/embeddings` directly (only reachable from the desktop — loopback-only), or an Ollama port.
...
curl -s http://10.0.0.243:11438/health        # passes through to Infinity
...
```bash
curl -s -X POST http://10.0.0.243:11437/control -d '{"mode":"yield"}'
# ... after gaming:
curl -s -X POST http://10.0.0.243:11437/control -d '{"mode":"auto"}'
```
...
```bash
curl -s http://10.0.0.243:11437/status | python3 -c "import json,sys; print(json.load(sys.stdin).get('tdarr'))"
```
