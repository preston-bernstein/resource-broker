...
| `BROKER_TDARR_GPU_WORKERS` | `1` | getint (>= 1) | `tdarr.New` (restore target; note `/status` `tdarr.gpu_workers` shows the LIVE count queried from Tdarr, not this configured value) | How many `transcodegpu` workers to restore after Yield or the internal-scraper-service window ends | Production **but NOT in the README table** | **`2`** |
