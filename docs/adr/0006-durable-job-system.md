...
**Feedback:** `GET /jobs/{id}` is the canonical, durable, restart-safe status read; `GET /jobs/{id}/events` is a thin SSE live layer over the same state; `GET /jobs?source=&owner=&state=` lists. The Broker is the **data plane only** — the human UX lives in the consumer that owns the relationship (internal-monitor-app's per-profile health page) and in Grafana; a Broker-native dashboard is roadmap, not built.
