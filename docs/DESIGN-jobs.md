...
- `GET /jobs?source=&owner=&state=` → a filtered list of Jobs, scoped by Consumer or state. This powers the fashion-monitor health page and Grafana dashboards.
...
- **J6** done — Metrics (queue depth, Job counts by state, run-time totals, terminal-outcome counters) exposed on `/metrics` and `/status`. The Grafana panels for the new `broker_jobs*` metrics, and the fashion-monitor adapter that submits Jobs, polls them, and shows Position/status on its per-profile health page, live in those other repos — **not in this repo**.
