...
- **J6** ✅ Metrics (queue depth, job states by state, run-time sum/count, terminal-outcome counters) in `/metrics` and `/status`. Grafana panels + the fashion-monitor adapter (submit + poll, surface position/status on the per-profile health page) live in those repos — **not in this repo**.
