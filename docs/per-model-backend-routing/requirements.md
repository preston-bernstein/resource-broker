...
- **estate-scraper** (vision image scoring) — sends vision requests (`qwen3-vl:30b`, `qwen2.5vl:7b`) via the Batch lane; must keep working unchanged.
- **fashion-monitor and other one-off CLI Consumers** — any Consumer speaking the Ollama API against either lane; unaffected unless the operator explicitly targets their model.
