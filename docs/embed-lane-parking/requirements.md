...
| estate-scraper (vision embeddings) | `:11438` embed lane (Infinity, image embeddings) | Incidentally covered — same `Gate` call site, same `Batch` class | Not the driver for this feature; benefits as a side effect of fixing `Gate` once. No estate-scraper-specific requirement here. |
| fashion-monitor | `:11436` Batch (scoring, via durable Job path per ADR-0006, not Synchronous) | **Not applicable** — Jobs don't go through `Gate`'s Synchronous path at all | Confirms out-of-scope: Job path is untouched. |
...
FR-11. Prometheus observability, extending `internal/metrics/metrics.go`'s existing conventions rather than inventing a new format (concrete names decided in C-8):
  ...
  - The `expired` outcome counter MUST be structured so a standard `rate(broker_requests_total{outcome="expired"}[5m]) > 0` Prometheus alert expression is sufficient — no additional derived metric needed. Documenting this exact expression in ADR-0009 and the README is itself a deliverable of this FR; provisioning the Alertmanager/Grafana rule that consumes it is cross-repo (home-infra owns shared Prometheus/Grafana config per the repo-architecture convention) and is not this repo's deliverable.
...
- Deploying the Prometheus alerting rule itself (home-infra's shared Grafana/Prometheus config, per the repo-architecture convention of shared-services-live-in-home-infra) — this repo's deliverable stops at exposing a metric the standard `rate(...) > 0` pattern can alert on, and documenting that expression (FR-11).
